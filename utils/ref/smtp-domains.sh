...
# 		done
		...
		# DELETE
		mx_records=$(curl -X GET "https://api.cloudflare.com/client/v4/zones/${cf_zone_id}/dns_records?type=MX&name=${domain_zone}&content=mxa.mailgun.org&content=mxb.mailgun.org&match=any" \
			-H "X-Auth-Email: nick@roundhouse-designs.com" \
			-H "X-Auth-Key: ${CLOUDFLARE_API_KEY}" \
			-H "Content-Type: application/json"\
			)
		...
		for row in $(echo "${mx_records}" | jq -r '.result[] | @base64'); do
			...
			echo "rec_type: ${rec_record_type}"
			echo "rec_value: ${rec_value}"
			
# 			curl -X DELETE "https://api.cloudflare.com/client/v4/zones/${cf_zone_id}/dns_records/372e67954025e0ba6aaa6d586b9e0b59" \
# 					-H "X-Auth-Email: nick@roundhouse-designs.com" \
# 					-H "X-Auth-Key: ${CLOUDFLARE_API_KEY}" \
# 					-H "Content-Type: application/json"
		done
