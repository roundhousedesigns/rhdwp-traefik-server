...
for d in /srv/www/*; do
	...
	# 		done
			...
			# DELETE
			# mxa.mailgun.org
			mx_records=$(curl -s -X GET "https://api.cloudflare.com/client/v4/zones/${cf_zone_id}/dns_records?type=MX&name=${virtual_host}&content=mxa.mailgun.org&match=all" \
				-H "X-Auth-Email: nick@roundhouse-designs.com" \
				-H "X-Auth-Key: ${CLOUDFLARE_API_KEY}" \
				-H "Content-Type: application/json"\
				)
			
			for row in $(echo "${mx_records}" | jq -r '.result[] | @base64'); do
				_jq() {
					echo "${row}" | base64 --decode | jq -r "${1}"
				}
				...
				rec_name=$(_jq '.name')
				rec_type=$(_jq '.type')
				rec_value=$(_jq '.content')
				
				echo "rec_name: ${rec_name} // ${rec_id}"
				echo "ZONE id: ${cf_zone_id}"
				
				curl -X DELETE "https://api.cloudflare.com/client/v4/zones/${cf_zone_id}/dns_records/${rec_id}" \
						-H "X-Auth-Email: nick@roundhouse-designs.com" \
						-H "X-Auth-Key: ${CLOUDFLARE_API_KEY}" \
						-H "Content-Type: application/json"
			done
			...
			# mxb.mailbun.org
			mx_records=$(curl -s -X GET "https://api.cloudflare.com/client/v4/zones/${cf_zone_id}/dns_records?type=MX&name=${virtual_host}&content=mxb.mailgun.org&match=all" \
				-H "X-Auth-Email: nick@roundhouse-designs.com" \
				-H "X-Auth-Key: ${CLOUDFLARE_API_KEY}" \
				-H "Content-Type: application/json"\
				)
